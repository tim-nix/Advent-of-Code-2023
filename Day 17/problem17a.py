...
# Since the path cannot travel more than 3 steps forward
# in a given direction, the set of visited must include
# both the directon of travel and the number of steps
...
def shortestPath(block, start, stop):
   ...
   while (len(minheap)> 0):
      ...
      if (current[1], current[2], current[3], current[4]) not in visited:
         ...
         # the directon or results in more than 3 steps in the same
         # direction (both of which are illegal).
         if current[3] == 'north':
            neighbors.pop(1)        # remove 'south'
            n_direction.pop(1)
            if current[4] == 3:     # if moved 3 forward:
            ...
            n_direction.pop(2)
            if current[4] == 3:     # if moved 3 forward:
               neighbors.pop(2)     # remove forward
               n_direction.pop(2)
         elif current[3] == 'west':
