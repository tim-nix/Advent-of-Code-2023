...
def countRemovable(bricks):
   ...
   # For each brick, count the total number of
   # bricks that would fall if the brick is
   # removed.
   count = 0
   for index in range(len(bricks)):
      falling = set()
      ...
      while len(toVisit) > 0:
         ...
         for s in supports[current]:
            if (s not in falling) and (supported[s].issubset(falling)):
               falling.add(s)
               toVisit.append(s)
               
      count += len(falling) - 1  # -1 because we do not count the
                                 # disintigrated brick
